...
Concat first_name and last_name column and order by 2nd column.

```SQL
SELECT actor_id, CONCAT(first_name, ' ', ;ast_name) AS FullName
FROM sakila.actor
ORDER BY 2;
```
...
Print out of sakila.actor table actor_id from rows between 16 and 25.

```SQL
SELECT *
FROM sakila.actor
ORDER BY actor_id
LIMIT 15, 10; /*10 is a offset value*/
```
